"""
Figure 4 Replication: Plaintext vs Sentence-Based CoT Comparison
Model: DeepSeek-R1-Distill-8B
Conditions: Plaintext (no-prompt) vs Sentence-Based CoT
...
def plot_single_layer_triplet(df, model_name, layer, task_order, output_path):
    ...
    panels = [
        ...
        (r"$\Delta$ Transfer Gain (Sentence-Based - Plain)", diff_mat, "coolwarm", -0.4, 0.4, r"$\Delta$ AUROC")
    ]
...
def plot_multi_layer_comparison(df, model_name, layers, task_order, output_path):
    ...
    for row_idx, layer in enumerate(layers):
        ...
        panels = [
            ...
            (rf"$\Delta$ Transfer (Sentence-Based - Plain) | Layer {layer}", diff_mat, "coolwarm", -0.4, 0.4, r"$\Delta$ AUROC")
        ]
...
def main():
    parser = argparse.ArgumentParser(description="Generate Figure 4 Plaintext vs Sentence-Based CoT Heatmaps")
